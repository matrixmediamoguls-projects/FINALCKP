...
import { Pause, Play, SkipBack, SkipForward, Shuffle, Repeat, Database, Radio, Activity } from "lucide-react";
...
const TIMELINE = ["Intro", "Verse 1", "Pre", "Hook", "Verse 2", "Hook", "Bridge", "Final"];
...
export default function VisualizerFrame({ act_id, track, audioData, error, playback }) {
  ...
  const title = track?.title || track?.name || "Reclamation Mainframe";
  const artist = track?.artist || "Musiq Matrix";
  const actLabel = track?.act || track?.album || track?.act_title || "Chroma Key Act Three";
  ...
  const intensity = Math.max(1, Math.min(100, Number(track?.intensity || track?.energy_score || 82)));
  const activeTimelineIndex = Math.min(TIMELINE.length - 1, Math.floor((progressPercent / 100) * TIMELINE.length));

  const spectrumBars = useMemo(() => Array.from({ length: 128 }, (_, index) => {
    const liveValue = Array.isArray(audioData) ? Number(audioData[index % audioData.length]) / 255 : 0;
    const idleValue = (30 + ((index * 41) % 70)) / 100;
    const value = isPlaying ? Math.max(0.2, liveValue || idleValue) : idleValue;
    return `${Math.round(value * 100)}%`;
  }), [audioData, isPlaying]);

  const waveformBars = useMemo(() => Array.from({ length: 88 }, (_, index) => {
    const liveValue = Array.isArray(audioData) ? Number(audioData[(index * 3) % audioData.length]) / 255 : 0;
    const idleValue = (16 + ((index * 23) % 70)) / 100;
  ...
  const visibleLyrics = useMemo(() => {
    ...
    if (!lines.length) {
      return [
        { id: "fallback-1", text: "I CREATE AS I SPEAK", active: true },
        { id: "fallback-2", text: "THE SYSTEM BENDS WHEN THE SIGNAL IGNITES" },
        { id: "fallback-3", text: "RECLAMATION IS NOT A REQUEST" },
        { id: "fallback-4", text: "POWER RETURNS TO THE SOURCE" },
        { id: "fallback-5", text: "FIRE IN MY VEINS, CODE IN MY HANDS" },
      ];
    }
    ...
    return lines.slice(start, start + 6).map((line, index) => ({ ...line, active: start + index === activeIndex }));
  }, [track?.lyric_lines, currentTime]);

  const carouselTracks = useMemo(() => {
    const source = tracks.length ? tracks : FALLBACK_TRACKS.map((name, index) => ({ title: name, index, duration: "4:08" }));
    ...
    return Array.from({ length: windowSize }, (_, slot) => {
      ...
      return { ...source[index], index, offset, distance: Math.abs(offset), isActive: offset === 0 };
    });
  }, [tracks, currentIndex]);

  if (error && !track) {
    return <div className="vf-shell vf-shell--error"><div className="vf-error">{error}</div></div>;
  }

  return (
    <div className="vf-shell">
      <audio ref={audioRef} src={track?.src} onTimeUpdate={handleTimeUpdate} onLoadedMetadata={handleLoadedMetadata} />
      ...
      <section className="vf-mainframe" aria-label="CKP Visualizer Core">
        <header className="vf-header">
          ...
          <div className="vf-status"><span>System</span><strong>Authorized</strong><i /></div>
        </header>
        ...
        <aside className="vf-panel vf-audio-feed">
          ...
          <div className="vf-mini-spectrum" aria-hidden="true">{waveformBars.slice(0, 44).map((height, index) => <span key={index} style={{ "--bar": height }} />)}</div>
        ...
        <main className="vf-core-zone">
          <div className="vf-reactor" style={{ "--intensity": intensity / 100 }}>
            ...
            <div className="vf-radial-spectrum" aria-hidden="true">
              {spectrumBars.map((height, index) => <span key={index} style={{ "--bar": height, "--angle": `${index * 2.8125}deg` }} />)}
            </div>
            <div className="vf-orbit vf-orbit-one" aria-hidden="true" />
            <div className="vf-orbit vf-orbit-two" aria-hidden="true" />
            <div className="vf-horizontal-wave" aria-hidden="true">{waveformBars.map((height, index) => <span key={index} style={{ "--bar": height }} />)}</div>
        ...
        <aside className="vf-right-stack">
          <section className="vf-panel vf-sync-core">
            <PanelTitle eyebrow="Real Time Protocol" title="Sync Core" />
            ...
            <StatusRow label="Chorus" value="Detected" />
          ...
          <section className="vf-panel vf-lyrics-feed">
            ...
            <div className="vf-lyrics-list">{visibleLyrics.map((line, index) => <p key={line.id || index} className={line.active ? "is-active" : ""}>{highlightKeywords(line.text)}</p>)}</div>
            <div className="vf-keywords"><span>POWER</span><span>RECLAMATION</span><span>SOVEREIGN</span><span>FIRE</span></div>
            <div className="vf-lyric-meter"><span>Lyrical Intensity</span><i style={{ width: `${intensity}%` }} /></div>
          </section>
        </aside>

        <section className="vf-artifact-card">
          <span>Sonic Artifact</span>
          <h1>{title}</h1>
          <p>{artist} // {actLabel}</p>
          <div className="vf-progress" onClick={handleSeek} role="button" tabIndex={0} onKeyDown={() => {}} aria-label="Seek track"><i style={{ width: `${progressPercent}%` }} /></div>
          <div className="vf-time"><span>{formatTime(currentTime)}</span><span>{formatTime(duration || track?.duration_seconds)}</span></div>
        </section>

        <section className="vf-timeline" aria-label="Protocol timeline">
          {TIMELINE.map((item, index) => <span key={`${item}-${index}`} className={index <= activeTimelineIndex ? "is-active" : ""}>{item}</span>)}
        </section>
        ...
        <section className="vf-jukebox" aria-label="Reclamation Jukebox">
          ...
          <div className="vf-card-rail">
            {carouselTracks.map((item) => (
              <button type="button" key={`${item.id || item.title}-${item.index}`} className={`vf-track-card ${item.isActive ? "is-active" : ""}`} style={{ "--offset": item.offset, "--distance": item.distance }} onClick={() => selectTrack?.(item.index, { autoplay: isPlaying })}>
        ...
        <nav className="vf-controls" aria-label="Playback controls">
          ...
          <button type="button" className="vf-controls__main" onClick={togglePlay} disabled={!track?.src} aria-label={isPlaying ? "Pause" : "Play"}>{isPlaying ? <Pause size={28} fill="currentColor" /> : <Play size={28} fill="currentColor" />}</button>
        ...
        <footer className="vf-bottom-rail">
          ...
          <span><Activity size={14} /> Visualizer Core</span>
...
function PanelTitle({ eyebrow, title }) { return <div className="vf-panel-title"><span>{eyebrow}</span><h2>{title}</h2></div>; }
function Metric({ label, value, level }) { return <div className="vf-metric"><span>{label}</span><i><b style={{ width: `${Math.max(0, Math.min(100, Number(level) || 0))}%` }} /></i><strong>{value}</strong></div>; }
function StatusRow({ label, value }) { return <div className="vf-status-row"><span>{label}</span><strong>{value}</strong></div>; }
function highlightKeywords(text = "") {
  const keywords = ["CODE", "SYSTEM", "KEYS", "MINE", "FIRE", "POWER", "RECLAMATION", "SOVEREIGN"];
  const pattern = new RegExp(`\\b(${keywords.join("|")})\\b`, "gi");
  return String(text).split(pattern).map((part, index) => keywords.includes(part.toUpperCase()) ? <mark key={index}>{part}</mark> : part);
}
