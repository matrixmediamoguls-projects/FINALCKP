...
export default function PairedTrackPortal({ sourceTracks, lyricAnchors, listenedTracks, selectedAnchorKey, onMarkListened, onAnchorSelect, onTrackError }) {
  ...
  return (
    <section className="rec-module-panel" aria-labelledby="paired-track-heading">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="rec-module-kicker">Scene 02 • Signal Keys</p>
        ...
        <span className="rec-module-badge is-hot" aria-live="polite">{listenedTracks.length}/{sourceTracks.length} signals received</span>
      </div>
              ...
              <div className="flex h-full flex-col justify-between gap-5">
                <div>
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <span className="rec-module-badge">{track.keyLabel}</span>
                    <span className={`rec-module-badge ${isComplete ? 'is-hot' : ''}`}>{isComplete ? 'Signal Received' : trackError ? 'Signal interrupted' : 'Awaiting Signal'}</span>
                  </div>
                  ...
                  <div className="mt-4 rounded-2xl border border-red-500/15 bg-red-950/10 p-3">
                    <span className="rec-module-micro text-red-200/70">Lyric anchor / text alternative</span>
                ...
                <button type="button" onClick={() => receiveTrack(track)} className="rec-module-action w-full justify-center">
      ...
      <div className="mt-5 rounded-3xl border border-white/10 bg-white/[0.03] p-4">
        <p className="rec-module-kicker">Signal Anchors</p>
        <div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-4" role="group" aria-label="Choose a signal anchor">
          {lyricAnchors.map((anchor) => {
            ...
            return (
              <button type="button" key={anchor.key} onClick={() => onAnchorSelect(anchor.key)} aria-pressed={active} className={`rec-module-card p-3 text-left transition ${active ? 'is-active' : ''}`}>
                <span className="rec-module-badge">{anchor.label}</span>
