export default function ModuleHeroCard({ moduleMeta, hasEntered, onEnter, actionLabel }) {
  return (
    <section className="rec-module-hero">
      ...
      <div className="flex flex-wrap items-start justify-between gap-5">
        <div className="max-w-3xl">
          <p className="rec-module-kicker">{moduleMeta.kicker}</p>
        ...
        <div className="min-w-[220px] rounded-2xl border border-red-400/25 bg-black/50 p-4 text-right">
          <span className="rec-module-micro block text-zinc-500">Module ID</span>
          <strong className="mt-1 block text-lg uppercase tracking-[0.18em] text-red-100">{moduleMeta.moduleId}</strong>
          <span className="rec-module-micro mt-4 block text-zinc-500">Identity State</span>
      ...
      <div className="mt-6 grid gap-3 md:grid-cols-3">
        {moduleMeta.statCards.map((item) => (
          <div key={item.label} className="rec-module-card p-4">
            <span className="rec-module-badge">{item.label}</span>
      ...
      {moduleMeta.learningObjectives?.length > 0 && (
        <section className="mt-6 rounded-2xl border border-red-500/20 bg-black/35 p-5" aria-labelledby="module-learning-objectives">
          <p id="module-learning-objectives" className="rec-module-kicker">Measurable learning objectives</p>
      ...
      <button type="button" onClick={onEnter} className="rec-module-action mt-7">
