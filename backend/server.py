...
# Uses Redis sorted sets when REDIS_URL is configured; falls back to an
# in-process dict for single-instance deployments.

PROTOCOL_CHAT_RATE_LIMIT = int(os.environ.get("PROTOCOL_CHAT_RATE_LIMIT", "20"))
PROTOCOL_CHAT_RATE_WINDOW = 3600  # seconds (1 hour)

_redis_client = None
_rate_limit_cache: dict[str, list[datetime]] = {}  # in-memory fallback
...
async def _get_redis():
    ...
    try:
        ...
        logger.warning(f"Rate limiter: Redis unavailable, using in-memory fallback ({exc})")
        _redis_client = None
    return _redis_client

async def check_protocol_rate_limit(user_id: str) -> tuple[bool, int, int]:
    """Return (allowed, remaining, reset_in_seconds).

    Counts how many LLM calls this user has made in the last PROTOCOL_CHAT_RATE_WINDOW
    seconds and either allows or rejects the incoming request.
    """
    now = datetime.now(timezone.utc)
    window_start = now - timedelta(seconds=PROTOCOL_CHAT_RATE_WINDOW)

    r = await _get_redis()

    if r:
        key = f"rate:protocol_chat:{user_id}"
        # Prune expired entries, count, then conditionally add
        pipe = r.pipeline()
        ...
        if count >= PROTOCOL_CHAT_RATE_LIMIT:
            oldest = await r.zrange(key, 0, 0, withscores=True)
            if oldest:
                reset_in = int(PROTOCOL_CHAT_RATE_WINDOW - (now.timestamp() - oldest[0][1])) + 1
            else:
                reset_in = PROTOCOL_CHAT_RATE_WINDOW
            return False, 0, reset_in

        # Record this request (unique member avoids collisions at same timestamp)
        member = f"{now.timestamp()}:{uuid.uuid4().hex[:8]}"
        await r.zadd(key, {member: now.timestamp()})
        await r.expire(key, PROTOCOL_CHAT_RATE_WINDOW + 60)
        remaining = PROTOCOL_CHAT_RATE_LIMIT - count - 1
        return True, remaining, PROTOCOL_CHAT_RATE_WINDOW

    else:
        # In-memory fallback — safe for single-process deployments
        timestamps = [t for t in _rate_limit_cache.get(user_id, []) if t > window_start]

        if len(timestamps) >= PROTOCOL_CHAT_RATE_LIMIT:
            oldest = min(timestamps)
            reset_in = int(PROTOCOL_CHAT_RATE_WINDOW - (now - oldest).total_seconds()) + 1
            _rate_limit_cache[user_id] = timestamps
            return False, 0, reset_in

        timestamps.append(now)
        _rate_limit_cache[user_id] = timestamps
        remaining = PROTOCOL_CHAT_RATE_LIMIT - len(timestamps)
        return True, remaining, PROTOCOL_CHAT_RATE_WINDOW
...
async def protocol_chat(data: ProtocolMessage, request: Request):
    ...
    allowed, remaining, reset_in = await check_protocol_rate_limit(user_id)
    if not allowed:
        reset_min = max(1, round(reset_in / 60))
        raise HTTPException(
            status_code=429,
            detail=f"Rate limit exceeded. You can send {PROTOCOL_CHAT_RATE_LIMIT} protocol messages per hour. Try again in {reset_min} minute{'s' if reset_min != 1 else ''}.",
            headers={
                "X-RateLimit-Limit": str(PROTOCOL_CHAT_RATE_LIMIT),
    ...
    act_elements = {1: "Earth", 2: "Water", 3: "Fire", 4: "Air"}
    act_names = {1: "The Fractured Veil", 2: "The Reflection Chamber", 3: "Reclamation", 4: "The Crucible Code"}
    act_themes = {
